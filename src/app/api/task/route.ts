...
export async function GET(request: Request) {
  ...
  const rawQuery = Prisma.sql`SELECT t.*,s.staffname,c.categoryname FROM tasks AS t LEFT JOIN staff AS s ON t.staffid = s.staffid LEFT JOIN categories AS c ON t.categoryid = c.categoryid order by taskid desc`;
...
export async function POST(request: Request) {
  const { staffid, clientname, categoryid, location, visitcount } =
    await request.json();
  ...
  try {
    await prisma.$transaction(async (tx) => {
      // 1. addnew task .
      for (let i = 0; i < parseInt(visitcount); i++) {
...
export async function PUT(request: Request) {
  const { taskid, staffid, clientname, categoryid, location, visitcount } =
    await request.json();
  let message: string = "SUCCESS";
  try {
