...
const TaskAddNew = (params: ParamTypes) => {
  ...
  const [clientname, setClientname] = useState(params.selRowData?.clientname ?? "");
  ...
  const [visitcount, setVisitcount] = useState(params.selRowData?.visitcount ?? "");

  const [staffOptionValues, setStaffOptionValues] = useState<{ value?: number | string; name?: string }[]>([{
    value: "no", name: "No Data"
  }])
  const [categoryOptionValues, setCategoryOptionValues] = useState<{ value?: number | string; name?: string }[]>([{
    value: "no", name: "No Data"
  }])
  ...
  const getStaffValues = () => {
    const fetchData = async () => {
      ...
      const staff_details = await fetch(
        "api/staff?=columns" + columns,
      );
      const res = await staff_details.json();
      const staffData: StaffObj[] = res.staff;

      //create option array
      const optionArray: { value?: number | string; name?: string }[] = staffData.map((s) => {
  ...
  const getCategoryValues = () => {
    const fetchData = async () => {
      const columns = JSON.stringify({ staffid: true, staffname: true })
      const catgegory_details = await fetch(
        "api/category",
      );
      ...
      //create option array
      const optionArray: { value?: number | string; name?: string }[] = categoryData.map((c) => {
